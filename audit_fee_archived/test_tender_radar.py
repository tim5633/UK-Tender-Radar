...
def parse_args() -> argparse.Namespace:
    ...
    default_key_file = Path.cwd() / "ch_api_key.txt"
    ...
    p.add_argument("--download-dir", default=str(root / "uk_accounts_pdfs"), help="Local PDF folder")
    p.add_argument(
        "--enable-ocr-fallback",
    ...
    p.add_argument("--history-csv", default=str(root / "test_tender_history.csv"), help="Detailed output CSV")
    p.add_argument("--shortlist-csv", default=str(root / "test_tender_shortlist.csv"), help="Priority shortlist CSV")
...
def main() -> int:
    ...
    if not api_key:
        print(
            ...
            "or create ch_api_key.txt in current folder."
        )
        return 1
